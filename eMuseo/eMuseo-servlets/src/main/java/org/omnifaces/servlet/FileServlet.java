 ...
 * @since 2.2
 */
public abstract class FileServlet<METADATA> extends HttpServlet {
	...
	protected abstract METADATA getMetadata(HttpServletRequest request) throws IllegalArgumentException;
	 ...
	 */
	protected abstract File getFile(HttpServletRequest request, METADATA metadata) throws IllegalArgumentException;
	 ...
	 */
	protected String getContentType(HttpServletRequest request, File file, METADATA metadata) {
	 ...
	 * type.
	 */
	protected boolean isAttachment(HttpServletRequest request, String contentType, METADATA metadata) {
	 ...
	 * @since 2.3
	 */
	protected String getAttachmentName(HttpServletRequest request, File file, METADATA metadata) {
		return file.getName();
	}
	 ...
	 */
	private boolean preconditionFailed(HttpServletRequest request, Resource<METADATA> resource) {
	 ...
	 * Set cache headers.
	 */
	private void setCacheHeaders(HttpServletResponse response, Resource<METADATA> resource, long expires) {
	 ...
	 */
	private boolean notModified(HttpServletRequest request, Resource<METADATA> resource) {
	 ...
	 */
	private List<Range> getRanges(HttpServletRequest request, Resource<METADATA> resource) {
	 ...
	 * Set content headers.
	 */
	private String setContentHeaders(HttpServletRequest request, HttpServletResponse response, Resource<METADATA> resource, List<Range> ranges) {
		String contentType = getContentType(request, resource.file, resource.metadata);
		String disposition = isAttachment(request, contentType, resource.metadata) ? "attachment" : "inline";
		String filename = encodeURI(getAttachmentName(request, resource.file, resource.metadata));
	 ...
	 */
	private void writeContent(HttpServletResponse response, Resource<METADATA> resource, List<Range> ranges, String contentType) throws IOException, FileNotFoundException {
	 ...
	 * @param <METADATA>
	 */
	private static class Resource<METADATA> {
		...
		public Resource(File file, METADATA metadata) {
